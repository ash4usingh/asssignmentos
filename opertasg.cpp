...
/**
Use this function to let the train know that passanger is on board.
Params:
  stattion: current station pointer
*/
void station_on_board(struct station *station)
...
void alarm(int seconds, const char *error_str)
{
	alarm_timeout = seconds;
	alarm_error_str = error_str;
	alarm(seconds,error_str);
}
...
int main()
{
	...
	alarm(1, "station_load_train() did not return immediately when no waiting passengers");
	...
	alarm(2, "station_load_train() did not return immediately when no free seats");
	station_load_train(&station, 0);
	alarm(0, NULL);
	...
	while (passengers_left > 0) {
		alarm(2, "Some more complicated issue appears to have caused passengers "
